...
<head>
 <title>Molecular Dynamics - Glycoprotein Builder - Choose Rotamers</title>
       ...
       <c:if test="${showPhiColumn}">
        ...
        <c:choose>
         <c:when test="${linkage.flexiblePhi}">
          ...
          <c:if test="${not empty linkage.phiValues}">
           <c:set var="first" value="${linkage.phiValues[0]}" />
           <c:choose>
            <c:when test="${first == 180.0}"><c:set var="tChecked" value="yes" /></c:when>
            <c:when test="${first == 60.0}"><c:set var="gChecked" value="yes" /></c:when>
            <c:when test="${first == -60.0}"><c:set var="minusGChecked" value="yes" /></c:when>
           </c:choose>
          </c:if>
          <td>
           <input type="radio" name="phi_${status.count + 2}" value="t"
                  <c:if test="${tChecked == 'yes'}">checked</c:if>
                  <c:if test="${linkage.phiSet}">disabled</c:if> />
          </td>
          <td>
           <input type="radio" name="phi_${status.count + 2}" value="g"
       ...
       <c:if test="${showOmegaColumn}">
        ...
        <c:choose>
         <c:when test="${linkage.flexibleOmega}">
          ...
          <c:if test="${not empty linkage.omegaValues}">
           <c:set var="first" value="${linkage.omegaValues[0]}" />
           <c:choose>
            <c:when test="${first == 180.0}"><c:set var="ggChecked" value="yes" /></c:when>
            <c:when test="${first == -60.0}"><c:set var="gtChecked" value="yes" /></c:when>
            <c:when test="${first == 60.0}"><c:set var="tgChecked" value="yes" /></c:when>
           </c:choose>
          </c:if>
          <td>
           <input type="radio" name="omega_${status.count + 2}" value="gg"
                  <c:if test="${ggChecked == 'yes'}">checked</c:if>
                  <c:if test="${linkage.omegaSet}">disabled</c:if> />
          </td>
          <td>
           <input type="radio" name="omega_${status.count + 2}" value="gt"
                  <c:if test="${gtChecked == 'yes'}">checked</c:if>
                  <c:if test="${linkage.omegaSet}">disabled</c:if> />
          </td>
          <td>
           <input type="radio" name="omega_${status.count + 2}" value="tg"
